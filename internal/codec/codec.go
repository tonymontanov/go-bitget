...
Also exposes two small helpers used across REST/WS parsers:
  ...
  - ParseDecimal: returns shopspring/decimal.Decimal from a string. Empty
    string maps to decimal.Zero (Bitget often sends "" for optional
    fields). Returns error on a malformed value.
  - ParseInt64:   returns int64 from a string in the same fashion. Empty
    string maps to 0.

The helpers exist because Bitget V2/V3 sends ALL numbers as strings, and the
...
// is used so encoding behaviour matches encoding/json (case-insensitive field
// matching disabled, RFC-compliant numeric handling). Same choice as in
// go-bybit / go-okx.
...
// ParseDecimal converts a Bitget numeric string into a decimal.Decimal.
// Empty input → decimal.Zero, no error. Used everywhere because Bitget
...
// ParseInt64 converts a Bitget numeric string to int64.
...
// ParseFloat64 converts a Bitget numeric string to float64.

...
func TestContract_ModifyOrder_Happy(t *testing.T) {
	...
	const fixture = `{
		...
		"data":{"orderId":"NEW-9999","clientOid":"core-uuid-2-new"}
	}`
	var rec requestRecorder
	var client *bitget.Client
	_, client = mockBitget(t, map[string]string{
	...
	// Caller supplies an explicit NewClientOrderID — most realistic
	// HFT case (idempotency + parent-strategy correlation).
	var info mixtypes.OrderInfo
	var err error
	info, err = mixOf(client).Trading().ModifyOrder(context.Background(), mixtypes.ModifyOrderRequest{
		...
		NewClientOrderID: "core-uuid-2-new",
	...
	if info.ClientOrderID != "core-uuid-2-new" {
	...
	if body["clientOid"] != "core-uuid-2" {
		t.Errorf("clientOid: want core-uuid-2 (existing order id), got %v", body["clientOid"])
	}
	if body["newClientOid"] != "core-uuid-2-new" {
		t.Errorf("newClientOid: want core-uuid-2-new (must differ from clientOid), got %v", body["newClientOid"])
	}
...
// TestContract_ModifyOrder_AutoGeneratedNewClientOid covers the
// fallback path: caller leaves NewClientOrderID empty, SDK auto-fills
// a `m-<32-hex>` token via crypto/rand. Critical regression test
// for the v1.0.3 PARTIUSDT bug where the SDK reused ClientOrderID
// for both fields and Bitget rejected with code=40786.
func TestContract_ModifyOrder_AutoGeneratedNewClientOid(t *testing.T) {
	t.Parallel()
	const fixture = `{
		"code":"00000","msg":"success","requestTime":0,
		"data":{"orderId":"NEW-1","clientOid":""}
	}`
	var rec requestRecorder
	var client *bitget.Client
	_, client = mockBitget(t, map[string]string{
	...
	var _, err = mixOf(client).Trading().ModifyOrder(context.Background(), mixtypes.ModifyOrderRequest{
		Symbol:        "BTCUSDT",
		ClientOrderID: "core-uuid-2",
		NewPrice:      decimal.RequireFromString("43500.0"),
		// NewClientOrderID intentionally left empty.
	})
	if err != nil {
		t.Fatalf("ModifyOrder: %v", err)
	}
	var _, body, _ = rec.snapshot()
	var newCOid, _ = body["newClientOid"].(string)
	if newCOid == "" {
		t.Fatal("newClientOid: SDK must auto-generate when caller leaves it empty")
	}
	if newCOid == "core-uuid-2" {
		t.Fatalf("newClientOid: must DIFFER from ClientOrderID, got %q (regression of v1.0.3 dup-clientOid bug)", newCOid)
	}
	if len(newCOid) < 10 {
		t.Errorf("newClientOid: looks too short (%q); expected `m-<32hex>`", newCOid)
	}
}

// TestContract_ModifyOrder_RejectsDuplicateClientOid covers the
// caller-misuse path where someone passes the same value for both
// fields explicitly. We fail fast client-side with a clear message
// so they don't burn an RTT on an inevitable code=40786.
func TestContract_ModifyOrder_RejectsDuplicateClientOid(t *testing.T) {
	t.Parallel()
	var client *bitget.Client
	_, client = mockBitget(t, map[string]string{}, nil)
	...
	var _, err = mixOf(client).Trading().ModifyOrder(context.Background(), mixtypes.ModifyOrderRequest{
		Symbol:           "BTCUSDT",
		ClientOrderID:    "core-uuid-2",
		NewClientOrderID: "core-uuid-2",
		NewPrice:         decimal.RequireFromString("43500.0"),
	})
	if err == nil {
		t.Fatal("expected ErrorKindInvalidRequest when NewClientOrderID == ClientOrderID")
	}
	if !bitget.IsInvalidRequest(err) {
		t.Fatalf("want ErrorKindInvalidRequest, got %v", err)
	}
...
func TestContract_ModifyBatchOrders_StillValidatesInputs(t *testing.T) {
	...
	var _, err = mixOf(client).Trading().ModifyBatchOrders(context.Background(), nil)
	if err == nil {

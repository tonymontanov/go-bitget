...
func TestContract_Public_Instruments(t *testing.T) {
	...
	// Live shape (2026-09-23): symbolType is the asset class, the contract
	// kind lives in "type". Guards against filtering perpetuals by SymbolType.
	if in.SymbolType != "crypto" || in.Type != "perpetual" {
		t.Fatalf("symbolType/type: want crypto/perpetual, got %q/%q", in.SymbolType, in.Type)
	}
